...
import {
  AlertCircle, ArrowLeft, ArrowRight, CalendarDays, Check, ChevronDown, CreditCard, Heart,
  MapPin, Minus, Plus, Search, ShoppingCart, Smartphone, Sun, Ticket,
  UserRound, MapPinned, Share2, BadgePercent, Download, Loader2,
} from "lucide-react";
...
function CheckoutPage({ counts, added, selectedSizes, total, onBack }: CheckoutProps) {
  ...
  const [payStage, setPayStage] = useState<"idle" | "sending" | "polling" | "failed">("idle");
  const [receipt, setReceipt] = useState("");
  const [errorMsg, setErrorMsg] = useState("");
  const [downloading, setDownloading] = useState(false);
  const [checkoutId, setCheckoutId] = useState("");
  const [pollAttempt, setPollAttempt] = useState(0);
  const [tickets] = useState<TicketLine[]>(() => ticketGroups.flatMap((t, i) => Array.from({ length: counts[i] ?? 0 }, () => ({ type: t.name, price: t.price, day: t.day.replace("Season pass · ", ""), id: makeTicketId() }))));

  useEffect(() => { window.scrollTo({ top: 0, behavior: "smooth" }); }, [submitted]);

  // Initiate STK Push
  const handlePay = async () => {
    if (!MpesaService.isValidPhone(buyer.phone)) {
    ...
    const result = await MpesaService.initiateSTKPush(buyer.phone, total, ref);

    if (!result.success || !result.checkoutRequestId) {
      setPayStage("failed");
      setErrorMsg(result.error || "Failed to send STK Push. Please try again.");
    ...
    pollPayment(result.checkoutRequestId);
  };

  // Poll for payment status (24 × 5s = 2 minutes)
  const pollPayment = (id: string) => {
    ...
    const check = async () => {
      if (attempts >= maxAttempts) {
        setPayStage("failed");
        setErrorMsg('Confirmation is taking longer than expected. If money was deducted, click "I Have Paid" to re-check.');
        toast.error("Payment confirmation timed out", { duration: 8000 });
        return;
      }
      attempts++;
  ...
  const manualCheck = async () => {
    ...
    toast.message("Re-checking payment status…");
    try {
      ...
      if (status === "completed") {
        ...
        toast.message("Still pending. Wait a moment and try again.");
      }
    } catch {
  ...
  const download = async () => {
    setDownloading(true);
    try { await downloadTickets({ name: buyer.name, poster: eventArt.url, venue: "Nyayo Stadium", tickets }); } finally { setDownloading(false); }
  };

  /* ── SUCCESS SCREEN ─────────────────────────────────────────────────── */
  if (submitted) {
    return <main className="min-h-screen bg-background text-foreground"><SiteHeader menuOpen={menuOpen} setMenuOpen={setMenuOpen} onHome={onBack}/><section className="mx-auto max-w-2xl px-4 py-14 text-center"><span className="mx-auto grid size-20 place-items-center rounded-full bg-success/15 text-success"><Check className="size-10"/></span><h1 className="mt-6 font-display text-5xl font-bold">Payment successful</h1><p className="mt-3 text-lg text-muted-foreground">M-Pesa confirmed Ksh. {total.toLocaleString()} from {buyer.phone}. Receipt <strong className="text-foreground">{receipt}</strong>.</p>
      <div className="mt-8 space-y-3 rounded-2xl border border-border bg-panel p-5 text-left">
        {tickets.map((t) => <div key={t.id} className="flex items-center gap-4 border-b border-border pb-3 last:border-0 last:pb-0"><img src={eventArt.url} alt="" className="size-14 rounded-lg object-cover"/><div className="min-w-0 flex-1"><p className="font-bold">{t.type}</p><p className="text-sm text-muted-foreground">{t.day} · ID {t.id}</p></div><Ticket className="size-5 text-warm"/></div>)}
        {added.length > 0 && <p className="pt-2 text-sm text-muted-foreground">Merch ({added.length}) — collect at the merch counter on match day.</p>}
      </div>
      <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">{tickets.length > 0 && <Button variant="ticket" size="pill" onClick={download} disabled={downloading}>{downloading ? <Loader2 className="animate-spin"/> : <Download/>} Download PDF ticket{tickets.length > 1 ? "s" : ""}</Button>}<Button variant="secondary" size="pill" onClick={onBack}>Back to event</Button></div>
    </section></main>;
  }
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto flex max-w-xl items-center justify-center px-4 py-8 sm:py-10">
        {["Details", "Payment", "Confirm"].map((step, i) => <div key={step} className="flex items-start"><div className="flex flex-col items-center"><span className={`grid size-10 place-items-center rounded-full text-sm font-bold ${i < 2 ? "bg-ticket" : "bg-panel text-muted-foreground"}`}>{i < 2 ? <Check className="size-5"/> : 3}</span><span className="mt-2 text-sm font-semibold">{step}</span></div>{i < 2 && <span className={`mt-5 h-px w-16 sm:w-28 ${i === 0 ? "bg-ticket" : "bg-border"}`}/>}</div>)}
      </div>
        ...
        <aside className="rounded-2xl border border-border bg-panel/50 p-5 sm:p-7 lg:sticky lg:top-28">
          ...
          <div className="mt-7 rounded-xl bg-panel p-4 sm:p-6">
            <div className="space-y-5">
              {ticketGroups.map((ticket, i) => { const quantity = counts[i] ?? 0; return quantity > 0 ? <div key={ticket.day} className="grid grid-cols-[64px_minmax(0,1fr)_auto] items-start gap-4 border-b border-border pb-5"><img src={eventArt.url} alt="Safari 7s" className="size-16 rounded-lg object-cover"/><div className="min-w-0"><p className="font-bold sm:text-lg">{ticket.name} (Ksh. {ticket.price.toLocaleString()})</p><p className="text-sm text-muted-foreground">Quantity: {quantity} Ticket(s)</p><p className="text-sm text-warm">{ticket.day}</p></div><p className="shrink-0 font-bold sm:text-lg">Ksh. {(ticket.price * quantity).toLocaleString()}</p></div> : null; })}
            </div>
            {added.length > 0 && <div className="border-b border-border py-5"><h2 className="text-lg font-bold">Merch</h2><p className="mb-3 text-sm text-muted-foreground">Collect at the merch counter on the day.</p>{merch.map((item) => added.includes(item.name) ? <div key={item.name} className="grid grid-cols-[64px_minmax(0,1fr)_auto] items-center gap-4 py-2"><img src={item.image} alt={item.name} className="size-16 rounded-lg bg-background object-cover"/><div className="min-w-0"><p className="font-bold">{item.name} ({selectedSizes[item.name]})</p><p className="text-sm text-muted-foreground">Quantity: 1</p></div><p className="shrink-0 font-bold">Ksh. {MERCH_PRICE.toLocaleString()}</p></div> : null)}</div>}
            <div className="flex items-center justify-between pt-5 text-xl font-bold sm:text-2xl"><span>Total:</span><span>Ksh. {total.toLocaleString()}</span></div>
          </div>
        </aside>

        <form className="space-y-7" onSubmit={(event) => { event.preventDefault(); if (agreed) handlePay(); }}>
          <section className="rounded-2xl border border-border bg-panel/50 p-5 sm:p-7">
            <h2 className="title-rule text-4xl font-bold">Your Details</h2><p className="mt-4 text-muted-foreground">Please fill in your information to complete the purchase</p>
            <div className="mt-7 space-y-5">
              <label className="block"><span className="mb-2 block text-sm font-bold uppercase tracking-[0.16em] text-muted-foreground">Full name <b className="text-destructive">*</b></span><input required name="name" value={buyer.name} onChange={(e) => setBuyer((b) => ({ ...b, name: e.target.value }))} className="h-14 w-full rounded-xl border border-input bg-background px-5 outline-none focus:ring-2 focus:ring-ring" placeholder="Enter your full name"/></label>
              <label className="block"><span className="mb-2 block text-sm font-bold uppercase tracking-[0.16em] text-muted-foreground">Email address <b className="text-destructive">*</b></span><input required type="email" name="email" value={buyer.email} onChange={(e) => setBuyer((b) => ({ ...b, email: e.target.value }))} className="h-14 w-full rounded-xl border border-input bg-background px-5 outline-none focus:ring-2 focus:ring-ring" placeholder="Enter your email address"/></label>
              <label className="block"><span className="mb-2 block text-sm font-bold uppercase tracking-[0.16em] text-muted-foreground">M-Pesa phone number <b className="text-destructive">*</b></span><input required type="tel" name="phone" pattern="[0-9+ ]{9,15}" value={buyer.phone} onChange={(e) => setBuyer((b) => ({ ...b, phone: e.target.value }))} className="h-14 w-full rounded-xl border border-input bg-background px-5 outline-none focus:ring-2 focus:ring-ring" placeholder="0712345678"/><small className="mt-2 block text-muted-foreground">Your M-Pesa number. An STK Push will be sent to this number.</small></label>
              <label className="block"><span className="mb-2 block text-sm font-bold uppercase tracking-[0.16em] text-muted-foreground">Discount code</span><span className="grid grid-cols-[minmax(0,1fr)_auto] gap-3"><input name="discount" className="h-14 min-w-0 rounded-xl border border-input bg-background px-5 outline-none focus:ring-2 focus:ring-ring" placeholder="Enter discount code (optional)"/><Button type="button" variant="secondary" className="h-14 px-6">Apply</Button></span></label>
              <label className="block"><span className="mb-2 block text-sm font-bold uppercase tracking-[0.16em] text-muted-foreground">Promo code</span><span className="relative block"><BadgePercent className="absolute left-5 top-1/2 size-5 -translate-y-1/2 text-muted-foreground"/><input name="promo" className="h-14 w-full rounded-xl border border-input bg-background pl-14 pr-5 outline-none focus:ring-2 focus:ring-ring" placeholder="Bought through an agent? Enter their code"/></span><small className="mt-2 block text-muted-foreground">Optional credits the sale to your agent. It doesn't change your total.</small></label>
              <label className="flex cursor-pointer items-center gap-3 text-base"><input required type="checkbox" checked={agreed} onChange={(event) => setAgreed(event.target.checked)} className="size-5 accent-[var(--ticket-strong)]"/><span>I agree to the <strong className="text-warm">Terms and Conditions</strong></span></label>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-border bg-panel/50 p-5 sm:p-7">
            ...
            <div className="mt-7">
              <label className="grid cursor-pointer grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-4 rounded-xl border border-ticket-strong bg-ticket/10 p-5 transition">
                <span className="grid size-12 place-items-center rounded-lg bg-success/10 text-success"><Smartphone className="size-6"/></span>
                <span><strong className="block text-lg">M-Pesa STK Push</strong><span className="text-sm text-muted-foreground">An M-Pesa prompt will be sent to your phone</span></span>
                <span className="grid size-8 place-items-center rounded-full bg-ticket"><Check className="size-5"/></span>
              </label>
            </div>
            <div className="mt-7 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              <Button type="button" variant="secondary" size="pill" onClick={onBack} disabled={payStage === "sending" || payStage === "polling"}><ArrowLeft/> Back</Button>
              <Button type="submit" variant="ticket" size="pill" disabled={payStage === "sending" || payStage === "polling"}>
                {payStage === "sending" ? <><Loader2 className="animate-spin"/> Sending…</> : payStage === "polling" ? <><Loader2 className="animate-spin"/> Waiting for M-Pesa…</> : <>Pay Ksh. {total.toLocaleString()} <ArrowRight/></>}
              </Button>
            </div>
          </section>
        </form>
      ...
      {payStage === "polling" && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-background/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="M-Pesa payment processing">
          <div className="w-full max-w-sm rounded-2xl border border-border bg-panel p-6 text-center shadow-2xl">
            <span className="mx-auto grid size-14 place-items-center rounded-full bg-success/15 text-success"><Smartphone className="size-7"/></span>
            <h3 className="mt-4 text-2xl font-bold">Processing Payment</h3>
            <div className="py-6">
              <Loader2 className="mx-auto size-10 animate-spin text-warm"/>
              <p className="mt-4 font-semibold">Check your phone for the M-Pesa prompt</p>
              <p className="mt-2 text-sm text-muted-foreground">Enter your M-Pesa PIN on your phone to complete the payment of <strong className="text-foreground">Ksh. {total.toLocaleString()}</strong>.</p>
              <p className="mt-3 text-xs text-muted-foreground/60">Do not close this page. Verifying payment… ({pollAttempt}/24)</p>
            </div>
            <button type="button" onClick={manualCheck} className="mt-2 text-sm underline text-muted-foreground hover:text-foreground cursor-pointer">
      ...
      {payStage === "failed" && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-background/80 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Payment status">
          <div className="w-full max-w-sm rounded-2xl border border-border bg-panel p-6 text-center shadow-2xl">
            <span className="mx-auto grid size-14 place-items-center rounded-full bg-destructive/15 text-destructive"><AlertCircle className="size-7"/></span>
            <h3 className="mt-4 text-2xl font-bold">Payment Status</h3>
            <p className="mt-3 text-sm text-muted-foreground px-2">{errorMsg}</p>
            <div className="mt-6 space-y-3">
              {checkoutId && (
                <Button variant="ticket" className="w-full" onClick={manualCheck}>
                  I Have Already Paid (Check Status)
                </Button>
              )}
              <div className="grid grid-cols-2 gap-3">
                <Button variant="secondary" onClick={() => setPayStage("idle")}>Cancel</Button>
                <Button variant="ticket" onClick={handlePay}>Try Again</Button>
              </div>
            </div>
          </div>
        </div>
